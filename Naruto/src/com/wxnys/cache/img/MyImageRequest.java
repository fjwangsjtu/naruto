 ...
 * decoded Bitmap.
 */
public class MyImageRequest extends Request<Bitmap> {
	...
	private static final String TAG = "StuImageRequest";
	 ...
	 */
	public MyImageRequest(String url, Response.Listener<Bitmap> listener,
	...
	private Response<Bitmap> doParse(NetworkResponse response) {
		...
		if (bitmap == null) {
			...
			if (isSampleSave) {
				...
				AppLogEx.v(TAG, "response.data.length" + response.data.length);
				byte[] sampledata = baos.toByteArray();
				AppLogEx.v(TAG, "sampledata.data.length" + sampledata.length);
				return Response.success(bitmap, MyImageHttpHeaderParser
						.parseCacheHeaders(response, sampledata));
			} else {
				return Response.success(bitmap, MyImageHttpHeaderParser
						.parseCacheHeaders(response, response.data));
			}
		}
	}
